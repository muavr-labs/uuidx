...
//msgp:ignore Version
//msgp:ignore Variant
//msgp:ignore URNPrefixError

const (
	DefaultBytesSize  = 16
	DefaultStringSize = 36
	DefaultURNSize    = DefaultStringSize + 9
)

// A UUID is a 128 bit (16 byte) Universal Unique IDentifier as defined in RFC
// 9562.
type UUID [DefaultBytesSize]byte
...
// Standard RFC9562 added V6 and V7 of UUID, but did not change specification of V1 and V4
...
const randPoolSize = DefaultBytesSize * 16
...
func ParseBytes(b []byte) (UUID, error) {
	var uuid UUID
	switch len(b) {
	case DefaultStringSize: // xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx
	case DefaultURNSize: // urn:uuid:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx
		if !bytes.EqualFold(b[:9], []byte("urn:uuid:")) {
			return uuid, URNPrefixError{string(b[:9])}
		}
		b = b[9:]
	case DefaultStringSize + 2: // {xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx}
		b = b[1:]
...
func (uuid UUID) String() string {
	var buf [DefaultStringSize]byte
	encodeHex(buf[:], uuid)
	return string(buf[:])
}
